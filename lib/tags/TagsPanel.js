...
export const TagsPanel = ({ notebookTracker }) => {
    ...
    return (React.createElement("div", { className: "ds-tags-panel" },
        ...
        hasNotebook && section === 'cell' && (React.createElement("div", { className: "ds-tags-cell-section" },
            ...
            React.createElement("div", { className: "ds-tags-presets" },
                React.createElement("button", { className: "ds-tags-presets-toggle", onClick: () => setShowPresets(p => !p), disabled: !activeCellRef.current },
                    React.createElement("span", null,
                        showPresets ? '▾' : '▸',
                        " Suggested tags")),
                showPresets && (React.createElement("div", { className: "ds-tags-presets-body" }, BUILT_IN_PRESETS.map(preset => (React.createElement("div", { key: preset.category, className: "ds-tags-preset-row" },
                    React.createElement("span", { className: "ds-tags-preset-category" }, preset.category),
                    React.createElement("div", { className: "ds-tags-preset-chips" }, preset.tags.map(t => {
                        const applied = activeTags.includes(t);
                        return (React.createElement("button", { key: t, className: `ds-tags-preset-chip${applied ? ' ds-tags-preset-chip--on' : ''}`, style: { '--tag-color': tagColor(t) }, onClick: () => {
                                if (!applied)
                                    applyTags([...activeTags, t]);
                            }, title: applied ? `Already applied` : `Add "${t}"`, disabled: applied },
                            applied && React.createElement("span", { className: "ds-tags-preset-check" }, "\u2713"),
                            t));
                    })))))))),
