            ...
            return (React.createElement(React.Fragment, { key: field.key },
                ...
                field.key === 'DS_COMPLETION_PROVIDER' ? (React.createElement("div", { className: "ds-settings-routing-controls" },
                    ...
                    React.createElement("label", { className: "ds-settings-token-label", title: "Max tokens returned per completion" },
                        "Tokens",
                        React.createElement("input", { className: "ds-settings-token-input", type: "number", min: 16, max: 2048, step: 16, value: (_c = values['COMPLETION_MAX_TOKENS']) !== null && _c !== void 0 ? _c : '128', onChange: e => handleChange('COMPLETION_MAX_TOKENS', e.target.value), title: "Max tokens returned per completion (default: 128)" })))) : field.type === 'text' ? (React.createElement("input", { className: "ds-settings-text-input", type: "text", value: (_d = values[field.key]) !== null && _d !== void 0 ? _d : '', placeholder: "e.g. claude-haiku-4-5 \u2014 leave blank for keyword matching", onChange: e => handleChange(field.key, e.target.value), title: "Model name within the active chat provider used for lightweight tasks (preference generation, selection). Leave blank to use keyword matching instead." })) : (React.createElement("select", { className: "ds-settings-select", value: (_e = values[field.key]) !== null && _e !== void 0 ? _e : '', onChange: e => handleChange(field.key, e.target.value) },
                    React.createElement("option", { value: "" }, "\u2014 select provider \u2014"),
                    PROVIDER_LIST.map(p => (React.createElement("option", { key: p, value: p }, p)))))));
...
const SettingsPanel = ({ apiClient, onClose, onSaved, notebookPath = '' }) => {
    ...
    return (React.createElement("div", { className: "ds-settings-outer" },
        ...
        topTab === 'models' ? (React.createElement(ModelsPanel, { apiClient: apiClient, onClose: onClose, onSaved: onSaved, notebookPath: notebookPath })) : topTab === 'mcp' ? (React.createElement(React.Fragment, null,
